...
extension WaiterDashboard on WMDashboard {
  ...
  List<Widget> hallWidget() {
    ...
    final hl = _model.hallFilter > 0 ? _model.halls.where((element) => element['f_id'] == _model.hallFilter) : _model.halls;
  ...
  List<Widget> tablesWidgets(int hall) {
    ...
    return [
      for (final t in tl) ...[
        InkWell(onTap:(){
          model.navigation.openWaiterTable(t['f_id']).then((value) => unlockTable(t['f_id']));
        }, child: Container(
  ...
  void unlockTable(int id) {
    BlocProvider.of<AppBloc>(prefs.context()).add(AppEventLoading('Wait',
        '/engine/waiter/order.php', {'action': 'unlocktable',
